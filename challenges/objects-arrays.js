...
const uni = [];
universities.forEach(function(x){
  if(x.includes('uni')) {
    uni.push(x)
  }
});
...
zooAnimals.forEach(function(name){
  animalNames.push(`Name: ${name.animal_name}, ${name.scientific_name}`)
})
...
const lowerCase = zooAnimals.map(function(name){
   return name.first_name.toLowerCase();
});
...
*/
const largerPopulation = zooAnimals.filter(function(thin){
  return thin.population < 5;
});
